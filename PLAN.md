...
> **Historical / superseded.** v0.5.0 pivoted this project to a **cross-platform
> CLI** with reporting through a local web server (`securitysight serve`); the
> pywebview desktop shell and PyInstaller packaging described below were removed.
> The architecture it established — the **SQLite store, OS-keychain secrets,
> REST API, and triage UI** — carried forward and is what the CLI is built on.
> Kept for the design rationale. See `CHANGELOG.md` (0.5.0) and `README.md`.
