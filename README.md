...
pcrm/
  ...
  pipeline.py        collect → ingest → score → alert
...
Early but functional (`v0.1`). The collector framework, data lake, scoring, Slack alerting, dashboard, and all three deployment paths work end to end, with a `pytest` suite covering the scoring correlations and credential redaction. The keyless collectors are exercised directly; the keyed ones are written to each provider's documented API and worth a field check on first live run. Natural next step: per-company Slack channel routing.
